...
def calculate():
	...
	i_num=1
	print('begin2')
	f2 = open('.\\data\\result.csv', 'w',encoding='utf-8')
	for doc in doc_list:
		...
		i_num+=1
def merge():
	df1=pd.read_csv('.\\data\\sorted2.csv',sep='\t',names=['time','content','authid','url','source'])
	df2=pd.read_csv('.\\data\\lshres.csv',sep='\t',names=['innovation'])
	df=pd.concat([df1,df2],1)
	df.to_csv('.\\data\\final.csv',sep='\t',header=None,index=None)
